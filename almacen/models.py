...
class VariacionManager(models.Manager):
    def colors(self):
        return super(VariacionManager, self).filter(variation_category='color', is_active=True)

    def sizes(self):
        return super(VariacionManager, self).filter(variation_category='size', is_active=True)
